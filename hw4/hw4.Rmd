...
```
  ...
  **1. Map the top 10 busiest airports. Size of dots should reflect the number of flights through that destination. Hint: You may find this tutorial on [Making Maps in R](http://eriqande.github.io/rep-res-web/lectures/making-maps-with-R.html) helpful.**
...
```
  ...
  It is difficult to distinguish the different sizes of the lines because the total number of flights in each route are close in number (range from about 11,000 to about 13,00) out of a total of about 116,000 flights. So, the proportion of the top 10 busiest routes range from 9% to 11%, which would be difficult to distinguish.
...
```
  ...
  - Another possible explanation for label 4 is around this time, there were plans to renovate the Tom Bradley International section in order to accommodate the growing need and demand for international flights. According to the Bureau of Transportation Statistics, after May 2005, demand in the flight market shifted towards having more international flights. So, while LAX was renovating the Tom Bradley International section, flights from LAX initially decreased because of this huge construction project (as these parts of the airport had to be closed), then flights to and from LAX went back up soon after renovations were finished.
   ...
   - Label 5 has an opposite effect from label 4 (there is an increase in flights). There is an increase in flights before the holidays (Christmas and New Years) because many people are traveling during this time.
...
model_data1 %>%
  ...
  summarize(description = min(description), distance = mean(distance), arrdelay = mean(arrdelay), depdelay = mean(depdelay)) %>%
...
```
  ...
  80% of the flight data was used for training and 20% of the data was set aside for testing (prediction). Three predictor variables (distance, departure delay and unique carrier) were used to predict the outcome variable (arrival delay).  Output shows values of coefficients.
  

```{r}

#Test data
test_flights_2008 <- flights_tbl %>%
  filter(origin == "LAX") %>% 
...
head(test_flights_2008)

#prediction values

  test_flights_2008 <- sdf_predict(test_flights_2008, ml2) %>%
    mutate(mse_sum = (prediction - arrdelay)^2) %>%
    collect

#calculate root mse by calculating sqrt(sum(y(predicted) - y(observed))^2/n)
  
  root_mse <- sqrt(sum(test_flights_2008$mse_sum) / count(test_flights_2008))
  root_mse
...
```
  ...
  The model from the training dataset is applied onto the test dataset. The test dataset is applying the model developed from the training dataset onto flights in 2008 in LAX. 
  - The root mean squared error (RMSE) of applying the model from the training dataset onto the test dataset is 14.25. A small RMSE is better because this means the model has less "error" in prediction.
  
  
```{r}
model_deciles <- lapply(model_partition1, function(x) {
  sdf_predict(ml2, x) %>%
...
deciles %>%
  ...
  labs(title = 'Average arrival delay by predicted decile for LAX', x = 'Decile', 
       y = 'Minutes')
...
carrier <- sdf_predict(ml2, test_flights_2008) %>%
...
```
  ...
  This plot shows the average arrival delay forecast. The regression line represents the predicted arrival delays and the points are the actual arrival delays.
...
```
  ...
  I found it interesting how MSP is one of the top 10 busiest airports and so wanted to see what the most popular destinations from MSP are. The map above shows the most popular destinations are on the West coast (i.e. California, Nevada, Arizona and Washington state). The top 10 destinations from MSP are ORD, DEN, DTW, DFW, ATL, STL, PHX, EWR, MDW and MEM.