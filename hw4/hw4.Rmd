...
```
  ...
  80% of the flight data was used for training and 20% of the data was set aside for testing (prediction). Five predictor variables (distance, departure delay, air time, departure time and unique carrier) were used to predict the outcome variable (arrival delay).  Output shows values of coefficients. *I did not run this code because I could not knit it into my Hw4 RMD file (it was taking too long and I would run into errors in connecting to Sparklyr database).
...
test_flights_2008 <- flights_tbl %>%
  ...
  left_join(airlines_tbl, by = c("uniquecarrier" = "code")) %>%
    select(year, month, arrdelay, depdelay, 
           distance, uniquecarrier, description, deptime, airtime)

test_flights_2008

#prediction values

test_data <- sdf_predict(test_flights_2008, ml2)
test_data
...
```
  ...
  Here, the model obtained from the training set is applied to the test set (predicting arrival delay for flights in 2008). Root mean squared error (RMSE) was calculated to assess model performance. Smaller RMSE values are better because it means the model has less error in its prediction performance. *Again, I did not run this code because I could not knit it into my Hw4 RMD file (it was taking too long and I would run into errors in connecting to Sparklyr database).
