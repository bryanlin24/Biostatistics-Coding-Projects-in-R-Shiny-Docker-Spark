...
In this improved plot, I removed missing values in the calculation for mean departure delay time and sorted these values. I also factor re-ordered the variable destination and the mean departure delay time. It also helped to change month to factor.
...
This plot shows that neither cut nor carat by itself predict price. These two variables are both important in determining price. Ideal cuts of diamonds are worth the most, but each cut is worth a high price.
...
A scatterplot is better because if we were to visualize this data with bin (i.e. use a histogram), like in Exercise 7.3.4, we see there are a few huge outliers in x and y, which skew the histogram to the right. However, using a scatterplot, we see the relationship between y and x is strongly linear, despite these huge outliers that skew the data to the right.
